...
*										    *
*	This version is for the SIMPLE-68008 single board computer.		            *
*	Original by lots of people (see below), but resurrected by Jeff Tranter (tranter@pobox.com)	*
*										    *
*************************************************************************************
*										    *
...
OPC   EQU DUART+26      * Output port config        (W)
INU   EQU DUART+26      * Input port (unlatched)    (R)
OPS   EQU DUART+28      * Output port Set           (W)
OPR   EQU DUART+30      * Output port Clear         (W)
...
* Use this value to run out of ROM
	ORG		$0C0800			* past the vectors in a real system
...
VEC_OUT
        ...
        LEA.L    SRA,A0     	* A0 points to console DUART
TXNOTREADY
        MOVE.B   (A0),D1        * Read DUART status
        BTST     #2,D1          * Test Tx RDY bit
        BEQ.S    TXNOTREADY     * Until DUART Tx ready
        MOVE.B   D0,TBA       	* Write character to send
...
VEC_IN
    ...
    LEA.L    SRA,A0      	* A0 points to console DUART
    MOVE.B   (A0),D1        * Read DUART status
    BTST     #0,D1          * Test Rx Data Ready bit
    BEQ.S    RXNOTREADY     * Branch If DUART Rx not ready
    MOVE.B   RBA,D0			* Read character received
    MOVEM.L  (A7)+,A0/D1    * Restore working registers
